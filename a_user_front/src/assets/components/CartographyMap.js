...
const CartographyMap = ({ hospitals, onHospitalUpdate, onHospitalAdd, language = 'fr' }) => {
  ...
  // L'utilisateur doit cliquer sur le bouton de géolocalisation
  ...
  const locateUser = useCallback(() => {
        ...
        (error) => {
          ...
          switch (error.code) {
            ...
            case error.POSITION_UNAVAILABLE:
              errorMsg = "Position indisponible. Assurez-vous que le GPS est activé.";
              break;
            case error.TIMEOUT:
      ...
      timeoutId = setTimeout(() => {
        ...
        if (bestPosition) {
          ...
          setLoadingLocation(false);
          alert("Impossible de localiser votre position. Veuillez réessayer.");
        }
  ...
  return typeMap[type] || 'MEDECIN_PEC';
    ...
    // 🔥 FONCTION POUR DÉMARRER LA MODIFICATION CORRIGÉE
     const handleEditHospital = async (hospital) => {
       ...
       try {
         const response = await fetch(`${process.env.REACT_APP_GATEWAY_URL || 'http://34.28.161.231:8080'}/api/hospitaux/${hospital.id}/prestataires`, {
           method: 'GET',
           headers: {
         ...
         if (response.ok) {
           ...
           if (hospitalWithPrestataires && Array.isArray(hospitalWithPrestataires.prestataires) && hospitalWithPrestataires.prestataires.length > 0) {
             ...
             const resp2 = await fetch(`${process.env.REACT_APP_GATEWAY_URL || 'http://34.28.161.231:8080'}/api/hospitaux/${hospital.id}/prestataires-only`, {
               method: 'GET',
               headers: {
                 'Content-Type': 'application/json',
                 'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      try {
        if ((!hospital.prestataires || hospital.prestataires.length === 0) && hospital.id) {
          ...
          const resp3 = await fetch(`${process.env.REACT_APP_GATEWAY_URL || 'http://34.28.161.231:8080'}/api/hospitaux/${hospital.id}/prestataires-only`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('token')}`
