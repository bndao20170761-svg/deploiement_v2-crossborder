  ...
  return typeMap[type] || 'MEDECIN_PEC';
    ...
    // 🔥 FONCTION POUR DÉMARRER LA MODIFICATION CORRIGÉE
     const handleEditHospital = async (hospital) => {
       ...
       try {
         const response = await fetch(`${process.env.REACT_APP_GATEWAY_URL || 'http://34.32.116.206:8080'}/api/hospitaux/${hospital.id}/prestataires`, {
           method: 'GET',
           headers: {
         ...
         if (response.ok) {
           ...
           if (hospitalWithPrestataires && Array.isArray(hospitalWithPrestataires.prestataires) && hospitalWithPrestataires.prestataires.length > 0) {
             ...
             const resp2 = await fetch(`${process.env.REACT_APP_GATEWAY_URL || 'http://34.32.116.206:8080'}/api/hospitaux/${hospital.id}/prestataires-only`, {
               method: 'GET',
               headers: {
                 'Content-Type': 'application/json',
                 'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      try {
        if ((!hospital.prestataires || hospital.prestataires.length === 0) && hospital.id) {
          ...
          const resp3 = await fetch(`${process.env.REACT_APP_GATEWAY_URL || 'http://34.32.116.206:8080'}/api/hospitaux/${hospital.id}/prestataires-only`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('token')}`
