   ...
   try {
     ...
     if (wizardData.id) {
       ...
       response = await axios.put(
         `${process.env.REACT_APP_GATEWAY_URL || 'http://34.28.161.231:8080'}/api/references/${wizardData.id}`,
         dto,
         {
           headers: {
       ...
       response = await axios.post(
         `${process.env.REACT_APP_GATEWAY_URL || 'http://34.28.161.231:8080'}/api/references/New_ref`,
         dto,
         {
           headers: {
             Authorization: `Bearer ${token}`,
