...
const DossierViewEnhanced = ({ patient, dossier: dossierProp, onBack, language }) => {
    ...
    const fetchDossier = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_GATEWAY_URL || 'http://34.32.116.206:8080'}/api/dossiers/${patient.codePatient}`,
          {
            headers: {
  ...
  useEffect(() => {
    if (patient?.dateNaissance) {
      ...
      setPatientAge(age);
      setIsMinor(age !== null && age <= 15);  // ⚠️ SEUIL À 15 ANS
    }
  }, [patient]);
    ...
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
            ...
            {patientAge !== null && (
              <div className="mt-2">
                <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${
                  isMinor ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'
                }`}>
                  {isMinor ? '👶 Dossier Pédiatrique' : '👤 Dossier Adulte'} - {patientAge} ans
                </span>
              </div>
            )}
          </div>
      ...
      {page && (
        <>
          ...
          {!isMinor && (
            <>
              {/* Informations Sociales - Plus détaillées pour adultes */}
...
export default DossierViewEnhanced;
