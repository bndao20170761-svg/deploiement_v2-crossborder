...
const AdvancedSearch = () => {
  ...
  const chargerFiltres = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_GATEWAY_URL || 'http://34.32.116.206:8080'}/api/hopitaux-proxy/actifs`,
        {
          headers: {
  ...
  const effectuerRecherche = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_GATEWAY_URL || 'http://34.32.116.206:8080'}/api/hopitaux-proxy/recherche?${params}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
