...
$backendServices = @(
    @{Name="api-register"; Path="api_register"; Port=8761},
    @{Name="api-configuration"; Path="api_configuration/demo"; Port=8888},
    @{Name="gateway-pvvih"; Path="Getway_PVVIH"; Port=8080},
    @{Name="forum-pvvih"; Path="Forum_PVVIH"; Port=9092},
    @{Name="gestion-user"; Path="gestion_user"; Port=9089},
    @{Name="gestion-reference"; Path="gestion_reference"; Port=9090},
    @{Name="gestion-patient"; Path="gestion_patient"; Port=9091}
)

$frontendServices = @(
    @{Name="gestion-forum-front"; Path="gestion_forum_front"; Port=3001},
    @{Name="a-reference-front"; Path="a_reference_front"; Port=3002},
    @{Name="a-user-front"; Path="a_user_front"; Port=3003}
)
...
function Build-Backend {
    ...
    try {
        ...
        Write-Host "  ✓ Pushed to Docker Hub: $imageName" -ForegroundColor Green
        return $true
...
function Build-Frontend {
    ...
    try {
        # Build Docker image (le Dockerfile fait le npm install et build)
...
if ($failCount -eq 0) {
    ...
    Write-Host "  .\deploy-from-dockerhub.ps1 -Pull -DockerHubUsername $DockerHubUsername" -ForegroundColor White
    Write-Host ""
    exit 0
} else {
    ...
    Write-Host "========================================" -ForegroundColor Red
    Write-Host ""
    Write-Host "Veuillez corriger les erreurs et réessayer." -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
