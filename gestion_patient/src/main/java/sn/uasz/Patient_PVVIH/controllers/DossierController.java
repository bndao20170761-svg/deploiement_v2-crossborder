...
import sn.uasz.referencement_PVVIH.dtos.DossierDTO;
...
public class DossierController {
    ...
    /**
     * ✅ Endpoint pour obtenir un dossier par codeDossier (pour Feign Client)
     */
    @PreAuthorize("hasRole('DOCTOR')")
    @GetMapping("/by-code/{codeDossier}")
    public DossierViewDto getDossierByCode(@PathVariable String codeDossier) {
        return dossierService.getDossierByCode(codeDossier);
    }

    /**
     * ✅ Endpoint pour obtenir tous les dossiers d'un patient (pour Feign Client)
     */
    @PreAuthorize("hasRole('DOCTOR')")
    @GetMapping("/by-patient/{codePatient}")
    public List<DossierViewDto> getDossiersByPatient(@PathVariable String codePatient) {
        return dossierService.getDossiersByPatient(codePatient);
    }

    /**
     * ✅ Endpoint pour obtenir un dossier avec infos patient complètes (pour Feign Client)
     */
    @PreAuthorize("hasRole('DOCTOR')")
    @GetMapping("/{codeDossier}/with-patient")
    public DossierViewDto getDossierWithPatient(@PathVariable String codeDossier) {
        return dossierService.getDossierWithPatient(codeDossier);
    }
